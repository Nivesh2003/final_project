...
QUESTIONS_FILE = Path(__file__).resolve().parent / "dummy_data" / "ai.json"
...
def admin_dashboard(request):
    ...
    context = {
        'students': students,
        'test_subject': questions_data.get('subject', ''),
...
def load_questions_data():
    """Load test question data from JSON file used by the exam page."""
    if not QUESTIONS_FILE.exists():
        return {"subject": "AI/ML", "questions": []}
...
def save_questions_data(data):
    """Persist test question data back to JSON file."""
...
def add_question(request):
    ...
    if correct_answer not in {"A", "B", "C", "D"}:
        messages.error(request, "Correct answer must be one of: A, B, C, D.")
        return redirect("admin_dashboard")
...
def update_question(request, question_id):
    ...
    if not all([text, option_a, option_b, option_c, option_d]):
        messages.error(request, "All question fields are required to update a question.")
        return redirect("admin_dashboard")
...
def delete_question(request, question_id):
    data = load_questions_data()
    questions = data.get("questions", [])
    filtered = [q for q in questions if q.get("id") != question_id]

    if len(filtered) == len(questions):
        messages.error(request, "Question not found.")
        return redirect("admin_dashboard")

    # Keep IDs sequential after deletion to keep templates/exam mapping predictable.
...
@staff_member_required(login_url='/admin/login/')
@require_POST
def update_test_subject(request):
    data = load_questions_data()
    subject = request.POST.get("subject", "").strip()

    if not subject:
        messages.error(request, "Test subject cannot be empty.")
        return redirect("admin_dashboard")
